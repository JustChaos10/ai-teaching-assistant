...
from fingers_counting_trails import FingerCounter

class NumericInputGame(BaseGame):
    def __init__(self, game_data, game_directory, app_stop_event=None):
        super().__init__(game_data, game_directory, app_stop_event, window_name=game_data.get('title', 'Numeric Input Game'))
        self.items = game_data.get('items', [])
        self.item_queue = []
        self.current_item = None
        self.input_processor = FingerCounter()
    ...
    def get_question_text(self, item) -> str:
        return item.get('question', 'How many fingers?')
