    ...
    def _init_pygame(self):
        try:
            ...
            try:
                ...
                print("[DEBUG] Pygame mixer initialized with alternative settings")
            except Exception as e2:
                print(f"[ERROR] Alternative pygame mixer init also failed: {e2}")
        ...
        for name, filename in sound_files.items():
            ...
            if os.path.exists(path) and os.path.getsize(path) > 0:
                try:
                    ...
                    print(f"[DEBUG] Loaded sound: {name}")
                except Exception as e:
