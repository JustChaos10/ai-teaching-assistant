...
class TeacherChatbot:
    ...
    # ---------------- TTS with Caching ----------------
    def _get_tts_cache_path(self, text, target_language):
        """Generate cache file path based on text hash and language"""
        text_hash = hashlib.md5(f"{text}_{target_language}".encode()).hexdigest()
        return OUTPUT_DIR / f"tts_cache_{text_hash}.wav"

    def tts(self, text, target_language="en"):
        # Check cache first
        cache_path = self._get_tts_cache_path(text, target_language)
        if cache_path.exists():
            print(f"[TTS] Using cached audio for text hash")
            return cache_path

        print(f"[TTS] Generating new audio...")
        ...
        print(f"[TTS] Audio cached for future use")
        return cache_path
    ...
    def pipeline(self, audio_path, language_hint=None):
        ...
        if self.image_generator:
            ...
            print(f"[Pipeline] Image generator not initialized, skipping images")
